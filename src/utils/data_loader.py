...
def _validate_spot_capacities(spot_cap: Any) -> None:
    """spot_capacities alanı dict[str, number] olmalı."""
    if not isinstance(spot_cap, dict):
        raise DataContractError(
            "spot_capacities bir dict olmalıdır."
        )
    for vtype, cap in spot_cap.items():
        if not isinstance(cap, (int, float)) or cap <= 0:
            raise DataContractError(
                f"spot_capacities['{vtype}'] pozitif sayı olmalıdır; "
                f"alınan: {cap!r}"
            )
...
def _validate_rental_routes(rental_routes: Any) -> None:
    ...
    for route_key, vehicles in rental_routes.items():
        ...
        for idx, v in enumerate(vehicles):
            ...
            if not isinstance(v["capacity_desi"], (int, float)) or v["capacity_desi"] <= 0:
                raise DataContractError(
                    f"rental_routes['{route_key}'][{idx}]['capacity_desi'] "
                    f"pozitif sayı olmalıdır."
                )


def _validate_cost_matrix(cost_matrix: Any, spot_capacities: dict) -> None:
    """
        ...
        for dest, vehicle_costs in destinations.items():
            ...
            for vtype, prices in vehicle_costs.items():
                if vtype not in known_types:
                    log.warning(
                        "cost_matrix['%s']['%s']['%s'] spot_capacities'de "
...
def _validate_schema(data: dict[str, Any]) -> None:
    ...
    _validate_spot_capacities(data["spot_capacities"])
    _validate_rental_routes(data["rental_routes"])
    _validate_cost_matrix(data["cost_matrix"], data["spot_capacities"])
...
def load_input(json_path: str | Path) -> dict[str, Any]:
    ...
    json_path : str | Path
        ...
        (örn. "logiai_mvp_input.json" ya da "/data/logiai_mvp_input.json")
    ...
    json.JSONDecodeError
        Dosya geçerli JSON değilse.
    DataContractError
        Şema ihlali varsa; mesajda hangi alan/yolun hatalı olduğu belirtilir.
    """
    path = Path(json_path)

    if not path.exists():
        raise FileNotFoundError(
            f"Girdi dosyası bulunamadı: '{path.resolve()}'\n"
            "İpucu: json_path'in doğru dizini gösterdiğinden emin olun."
        )
    ...
    with path.open(encoding="utf-8") as fh:
        try:
            ...
            raise json.JSONDecodeError(
                f"JSON ayrıştırma hatası [{path.name}]: {exc.msg}",
                exc.doc,
                exc.pos,
            ) from exc
    ...
    # Tarihler ve toplam talep miktarı hakkında özet log
